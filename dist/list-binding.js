...
import { xin } from './xin';
import { cloneWithBindings, elementToBindings, BOUND_CLASS, tosiValue, tosiPath, LIST_BINDING_REF, LIST_INSTANCE_REF, registerArrayIdPath, } from './metadata';
    ...
    for (const boundElement of boundElements) {
        ...
        for (const binding of bindings) {
            ...
            if (binding.binding.toDOM != null) {
                ...
                binding.binding.toDOM(boundElement, xin[binding.path], binding.options);
            }
        }
    }
}
...
export class ListBinding {
    ...
    constructor(boundElement, value, options = {}) {
        ...
        const templateEl = Array.from(boundElement.children).find((c) => c instanceof HTMLTemplateElement);
        let insertionRef = null; // where to insert padding/items
        if (templateEl != null) {
            ...
            if (templateEl.content.children.length < 1 ||
                ...
                throw new Error(`ListBinding expects a template with exactly ${expectedChildren} child element(s)`);
            }
            this.templates = Array.from(templateEl.content.children).map((child) => cloneWithBindings(child));
            ...
            this.templates = [boundElement.children[0]];
            this.templates[0].remove();
        }
        else {
            throw new Error('ListBinding expects a <template> child or exactly one child element');
        }
        ...
        if (itemsPerRow > 1) {
            ...
            const style = this.boundElement.style;
            if (style != null) {
    ...
    visibleSlice() {
        ...
        if (virtual != null && this.boundElement instanceof HTMLElement) {
            ...
            const visibleColumns = virtual.width != null
            ...
            if (virtual.minHeight != null) {
                ...
                const visibleRows = Math.ceil(viewportHeight / effectiveHeight) +
                ...
                topBuffer = scrollTop;
                bottomBuffer = Math.max(0, totalScrollHeight - scrollTop - viewportHeight);
                return {
                    items: visibleArray,
                    firstItem,
                    lastItem,
                ...
                const visibleRows = Math.ceil(viewportHeight / virtual.height) +
                    (virtual.rowChunkSize || 1);
                const visibleItems = visibleColumns * visibleRows;
                ...
                bottomBuffer = Math.max((totalRows - visibleRows) * virtual.height - topBuffer, 0);
            }
        }
        return {
            items: visibleArray,
    ...
    update(array, isSlice) {
        ...
        const { hiddenProp, visibleProp } = this.options;
        const arrayPath = tosiPath(array);
        ...
        if (this.options.virtual != null) {
            this.boundElement.setAttribute('aria-rowcount', String(slice.items.length));
        }
        const previousSlice = this._previousSlice;
        const { firstItem, lastItem, topBuffer, bottomBuffer } = slice;
        const sliceUnchanged = hiddenProp === undefined &&
        ...
        const elements = [];
        const seenIds = idPath != null ? new Set() : null;
        for (let i = firstItem; i <= lastItem; i++) {
            const item = slice.items[i];
            if (item === undefined) {
                continue;
            ...
            if (seenIds != null && idPath != null) {
                ...
                if (seenIds.has(id)) {
                    if (!warnedDuplicateListId) {
                        ...
                        console.error(`tosijs list binding: duplicate idPath value "${id}" (idPath: "${idPath}"). ` +
            ...
            if (group == null) {
                created++;
                const rawItem = tosiValue(item);
                group = this.templates.map((tmpl) => cloneWithBindings(tmpl));
                ...
                if (idPath != null) {
                    ...
                    for (const el of group)
                        updateRelativeBindings(el, itemPath);
        ...
        // supported.)
        let insertionPoint = this.listTop;
    ...
    _updateInterpolatedBuffers(slice) {
        const { t, position, scrollTop, viewportHeight, totalScrollHeight, rowHeight, } = slice.interpolation;
        // Measure actual rendered content height from DOM
        let renderedHeight = 0;
        for (const child of Array.from(this.boundElement.children)) {
            if (child === this.listTop || child === this.listBottom)
                continue;
            renderedHeight += child.offsetHeight || rowHeight;
        }
        ...
        const offset = Math.max(0, t * pinBottom + (1 - t) * pinTop - (position % 1) * rowHeight);
        if (this.listTop != null && this.listBottom != null) {
...
export const getListBinding = (boundElement, value, options) => {
...
export const getListInstance = (element) => {
    let item;
    while (!(item = element[LIST_INSTANCE_REF]) &&
        element &&
...
export const deleteListItem = (element) => {
    const instance = getListInstance(element);
    if (!instance) {
        console.error('deleteListItem failed, element is not part of a list instance', element);
        return false;
    }
    const binding = getListBinding(instance.element.parentElement);
    if (!binding.options.idPath) {
        console.error('deleteListItem failed, list binding has no idPath', element.parentElement, binding);
        return false;
    }
...
const POSITION_TO_BLOCK = {
    start: 'start',
    middle: 'center',
    end: 'end',
    nearest: 'nearest',
};
export const scrollListItemIntoView = (element, item, options = {}) => {
    const binding = getListBinding(element);
    if (binding == null) {
        console.error('scrollListItemIntoView failed, element has no list binding', element);
        return false;
    }
    ...
    if (virtual != null && element instanceof HTMLElement) {
        ...
        if (virtual.minHeight != null) {
            ...
            switch (position) {
                case 'start':
                    ...
                    break;
                case 'end':
                    scrollTarget = Math.max(0, ((itemRow - visibleRows + 1) / maxTopRow) * maxScrollTop);
                    break;
                case 'nearest': {
                    const currentScroll = useWindowScroll
                        ? Math.max(0, -element.getBoundingClientRect().top)
                        : element.scrollTop;
                    const currentFraction = maxScrollTop > 0 ? currentScroll / maxScrollTop : 0;
            ...
            switch (position) {
                ...
                case 'nearest': {
                    ...
                    else if (itemTop + virtual.height >
        ...
        if (domGroup == null || domGroup.length === 0) {
            console.error('scrollListItemIntoView failed, no DOM element found for item', item);
            return false;
        }
