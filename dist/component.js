...
const checkValueContract = (el, newValue) => {
    const cls = el.constructor;
    if (!Object.prototype.hasOwnProperty.call(cls, 'contract'))
        return;
    const schema = cls.contract?.value;
    if (schema == null)
        return;
    const err = contractViolation(newValue, schema);
    if (err != null) {
        throw new TypeError(`<${el.tagName?.toLowerCase()}> value contract violation: ${err}`);
    }
};
...
const derivedInitAttributes = new WeakMap();
const attributeNudgeGiven = new Set();
function anonElementTag() {
...
const warnedShadowContentBindings = new Set();
// Component classes already warned about class fields shadowing initAttributes
const warnedFieldShadowedAttrs = new Set();
...
const handlerCollisionChecked = new WeakSet();
// Lazy shared MutationObserver for deprecated initAttributes
let legacyAttributeObserver = null;
function getLegacyAttributeObserver() {
...
function setGlobalStyle(tagName, styleSpec) {
...
function insertGlobalStyles(tagName) {
    if (globalStyleSheets[tagName]) {
        document.head.append(elements.style({ id: tagName + '-component' }, globalStyleSheets[tagName]));
    }
...
function capturePartsFrom(content) {
...
export class Component extends HTMLElement {
    ...
    static _resolveInitAttributes() {
        ...
        if (ownContract?.attributes != null) {
            if (ownInit != null) {
                throw new Error(`${this.name} declares BOTH static initAttributes AND ` +
            ...
            if (missingDefaults.length > 0) {
                throw new Error(`${this.name} contract.attributes entries missing 'default': ` +
        ...
        if (ownInit != null &&
            ...
            console.warn(`${this.name} declares a contract AND a separate static ` +
    ...
    static elementCreator(options = {}) {
        ...
        if (!Object.prototype.hasOwnProperty.call(componentClass, '_elementCreator')) {
            // Deprecation warnings for options-based API
            if (options.tag !== undefined) {
                warnDeprecated('elementCreator-tag', 'Passing tag to elementCreator() is deprecated. Use static preferredTagName instead.');
            }
            if (options.styleSpec !== undefined) {
                warnDeprecated('elementCreator-styleSpec', 'Passing styleSpec to elementCreator() is deprecated. Use static lightStyleSpec instead.');
            }
            if (options.extends !== undefined) {
                warnDeprecated('elementCreator-extends', 'Passing extends to elementCreator() is deprecated. Use static extends instead.');
            }
            // Resolve tag: options.tag > static preferredTagName > camelToKabob > anon
            let tagName = options.tag ?? componentClass.preferredTagName;
            if (tagName == null) {
                if (typeof componentClass.name === 'string' &&
                    componentClass.name !== '') {
            ...
            const defineOptions = extendsTag
                ? { extends: extendsTag }
                : undefined;
            ...
            if (proto != null &&
                ...
                const inner = proto.connectedCallback;
            ...
            window.customElements.define(tagName, this, defineOptions);
    ...
    initAttributes(...attributeNames) {
        warnDeprecated('initAttributes', 'initAttributes() is deprecated. Use static initAttributes = { ... } instead.');
    ...
    _partsCache = Object.create(null);
    get parts() {
                ...
                get(_target, ref) {
                    ...
                    if (element == null) {
                        ...
                        if (element == null) {
                            ...
                            if (legacy != null) {
                                warnDeprecated('data-ref', 'data-ref is deprecated and will be removed in tosijs 1.8.0 — use part="…" instead.');
    ...
    constructor() {
        ...
        const initAttrs = this.constructor._resolveInitAttributes();
        if (initAttrs) {
    ...
    _warnOnHandlerCollisions() {
        const ctor = this.constructor;
        if (handlerCollisionChecked.has(ctor))
            return;
        ...
        queueMicrotask(() => {
            ...
            const usesLegacyOnResize = names.delete('onResize') && this.handleResize === undefined;
            if (usesLegacyOnResize) {
                console.warn(`<${tag}> defines 'onResize', which is deprecated. Rename it to ` +
            ...
            console.warn(`<${tag}> defines ${list}. The elements factory treats on<Event> property ` +
    ...
    _drainPendingAttrOps() {
        ...
        for (const op of queue) {
            if (this.hasAttribute(op[1]))
                preExisting.add(op[1]);
        }
        for (const op of queue) {
            if (op[0] === 'set') {
                if (!preExisting.has(op[1]))
                    this.setAttribute(op[1], op[2]);
            }
            else {
    ...
    // be "restored")
    _installedAttrAccessors;
    _setupAttributeAccessors(initAttrs) {
        if (!this._attrValues) {
        ...
        for (const attrName of Object.keys(initAttrs)) {
            ...
            if (typeof defaultValue === 'object' && defaultValue !== null) {
                console.warn(`${this.tagName}: initAttributes.${attrName} is an object. Use a regular property instead.`);
                continue;
            }
            ...
            if (defaultValue === true) {
                throw new Error(`${this.tagName}: static initAttributes.${attrName} defaults a boolean attribute to true, ` +
    ...
    _installAttrAccessor(attrName, attrKabob, defaultValue) {
    ...
    _recoverShadowedAttrAccessors() {
        const installed = this._installedAttrAccessors;
        const initAttrs = this.constructor._resolveInitAttributes();
        if (installed == null || initAttrs == null)
            return;
        const shadowed = [];
        for (const attrName of installed) {
            ...
            this._installAttrAccessor(attrName, camelToKabob(attrName), initAttrs[attrName]);
        ...
        if (shadowed.length > 0 && !warnedFieldShadowedAttrs.has(this.tagName)) {
            ...
            console.warn(`<${this.tagName.toLowerCase()}> declares instance field(s) ${list} ` +
    ...
    hydrate() {
        if (!this._hydrated) {
            ...
            let _content = typeof this.content === 'function'
            ...
            if (Array.isArray(_content)) {
                ...
                for (const key of Object.keys(hostProps)) {
                    elementSet(this, key, hostProps[key]);
                }
            }
            const ctor = this.constructor;
            const shadowStyle = ctor.shadowStyleSpec ?? ctor.styleSpec;
            if (ctor.styleSpec && !ctor.shadowStyleSpec) {
                warnDeprecated('static-styleSpec', 'static styleSpec is deprecated. Use static shadowStyleSpec instead.');
            }
            let { styleNode } = ctor;
            if (shadowStyle) {
            ...
            if (this.styleNode) {
                console.warn(this, 'styleNode is deprecated, use static shadowStyleSpec instead');
            ...
            if (styleNode) {
                ...
                if (!warnedShadowContentBindings.has(this.tagName) &&
                    shadow.querySelector(BOUND_SELECTOR) != null) {
                    warnedShadowContentBindings.add(this.tagName);
                    console.warn(`<${this.tagName.toLowerCase()}> has data-binding sugar in its ` +
                ...
                appendContentToElement(this, _content, cloneElements);
                ...
                if (existingChildren.length > 0) {
                    const slotMap = { '': this };
                    Array.from(this.querySelectorAll('tosi-slot,xin-slot')).forEach((slot) => {
    ...
    validateValue() {
        if (!this.internals || this.value === undefined)
            return;
        const value = typeof this.value === 'string' ? this.value : String(this.value);
        validateAgainstConstraints(this, value);
    }
}
class TosiSlot extends Component {
...
class XinSlot extends Component {
    ...
    constructor() {
        super();
        warnDeprecated('xin-slot', '<xin-slot> is deprecated. Use <tosi-slot> instead.');
    }
