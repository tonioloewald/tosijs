...
const inboundPaths = new Set();
function findSyncedRoot(syncedPaths, changedPath) {
...
function isInbound(changedPath) {
...
function applyInbound(path, value) {
...
export async function sync(transport, options, ...proxies) {
    ...
    const flushOutbound = throttle(() => {
        ...
        try {
            ...
            console.error('sync: transport.send failed; deltas requeued and will retry on the next flush', e);
        }
    }, interval);
    // Register inbound handler
    transport.onReceive((messages) => {
        for (const msg of messages) {
            if (findSyncedRoot(syncedPaths, msg.path) === undefined)
                continue;
    ...
    for (const proxy of proxies) {
        ...
        if (path === undefined) {
            throw new Error('sync() requires boxed proxies or string paths. Got a non-proxy value.');
        }
        syncedPaths.add(path);
        const listener = observe((changedPath) => changedPath === path || changedPath.startsWith(path + '.'), (changedPath) => {
            if (isInbound(changedPath))
                return;
            if (findSyncedRoot(syncedPaths, changedPath) === undefined)
                return;
