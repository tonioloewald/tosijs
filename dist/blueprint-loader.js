...
async function settleBlueprints(host, selector, loaderTag) {
    const blueprintElements = Array.from(host.querySelectorAll(selector)).filter((elt) => elt.src);
    const results = await Promise.allSettled(blueprintElements.map((elt) => elt.packaged()));
    for (let i = 0; i < results.length; i++) {
        const result = results[i];
        if (result.status === 'rejected') {
            console.error(`<${loaderTag}> failed to load blueprint from "${blueprintElements[i].src}":`, result.reason);
        }
    }
}
...
class DeprecatedBlueprint extends Blueprint {
    ...
    constructor() {
        super();
        warnDeprecated('xin-blueprint', '<xin-blueprint> is deprecated. Use <tosi-blueprint> instead.');
    }
}
...
class DeprecatedLoader extends Component {
    ...
    constructor() {
        super();
        warnDeprecated('xin-loader', '<xin-loader> is deprecated. Use <tosi-loader> instead.');
    }
    async load() {
