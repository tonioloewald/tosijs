...
const freshDuringRestore = new Set();
...
export function setShareStore(s) {
    store = s;
}
// --- IndexedDB helpers ---
function openDB() {
...
// --- Helpers ---
function isShareMessage(data) {
    return (data != null &&
        data.type === 'tosijs-share' &&
        typeof data.path === 'string');
}
function findSharedRoot(changedPath) {
...
function initChannel() {
    ...
    channel.onmessage = (event) => {
        ...
        if (!isShareMessage(msg))
            return;
        if (msg.origin === origin)
            return;
        // Check that the message path falls under a shared root
        if (findSharedRoot(msg.path) === undefined)
            return;
...
function schedulePersist(rootPath) {
    if (!debouncedSaves.has(rootPath)) {
        debouncedSaves.set(rootPath, debounce(() => {
...
export async function share(...proxies) {
    ...
    for (const proxy of proxies) {
        ...
        if (path === undefined) {
            throw new Error('share() requires boxed proxies or string paths. Got a non-proxy value.');
        }
        ...
        observe((changedPath) => changedPath === path || changedPath.startsWith(path + '.'), (changedPath) => {
