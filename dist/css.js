...
export function StyleSheet(id, styleSpec) {
    const spec = tosiValue(styleSpec);
    ...
    if (path !== undefined) {
        observe(path, () => {
            element.textContent = css(tosiValue(styleSpec));
...
const alwaysUnitless = /^(animation-iteration-count|column-count|flex(-grow|-shrink)?|font-weight|opacity|order|orphans|scale|tab-size|widows|z-index|zoom)$/;
const dualUnitless = /^line-height$/;
export const processProp = (prop, value) => {
    ...
    const unitlessHere = alwaysUnitless.test(bareProp) ||
...
const renderProp = (indentation, cssProp, value) => {
    if (value === undefined) {
        return '';
    }
...
const renderStatement = (key, value, indentation = '') => {
    ...
    if (typeof value === 'object' && !(value instanceof Color)) {
        const renderedRule = Object.keys(value)
            .map((innerKey) => renderStatement(innerKey, value[innerKey], `${indentation}  `))
            .join('\n');
...
export const initVars = (obj) => {
    warnDeprecated('initVars', 'initVars is deprecated. Just use _ and __ prefixes instead.');
    const rule = {};
...
export const invertLuminance = (map) => {
    const inverted = {};
...
export const varDefault = new Proxy({}, {
    get(target, prop) {
...
export const vars = new Proxy({}, {
    get(target, prop) {
        ...
        if (target[prop] == null) {
            ...
            const [, _varName, , isNegative, scaleText, method] = prop.match(/^([-\w]*?)((_)?(\d+)(\w?))?$/) || ['', prop];
            const varName = `--${_varName}`;
            if (scaleText != null) {
                const scale = isNegative == null
...
export function getThemePreferences() {
    const mq = (query) => typeof matchMedia !== 'undefined' && matchMedia(query).matches;
    return {
...
export function onThemePreferencesChange(callback) {
