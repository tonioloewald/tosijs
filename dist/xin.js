...
import { touch, observe as _observe, unobserve, updates, } from './path-listener';
import { getByPath, setByPath } from './by-path';
import { getBind, getOn } from './registry';
import { elements } from './elements';
import { tosiValue, XIN_VALUE, XIN_PATH, XIN_OBSERVE, XIN_BIND, XIN_ON, TOSI_ACCESSOR, TAKE_DESCRIPTOR, LIST_INSTANCE_REF, } from './metadata';
...
function box(x, path) {
    ...
    return new Proxy(boxedScalarTarget, regHandler(path, true));
}
const listElement = () => new Proxy({}, regHandler('^', true));
// List operation helpers
const selectorToFieldPath = (selector) => {
...
const findByField = (target, fieldPath, value) => {
    for (let i = 0; i < target.length; i++) {
        if (`${getByPath(target[i], fieldPath)}` === `${value}`)
            return i;
    }
    return -1;
};
const makeListMethods = (path, target) => ({
    listFind(selectorOrElement, value) {
...
function warnDeprecation() {
    if (!deprecationWarned) {
        console.warn('xinValue, tosiValue, xinPath, tosiPath, etc. are deprecated. Use .tosi.value, .tosi.path, .tosi.observe(), etc. instead.');
        deprecationWarned = true;
    }
}
// Check if target is a boxed scalar proxy
const isBoxedScalar = (target) => {
    return target === boxedScalarTarget;
};
// Accessor handler — wraps the same target, dispatches accessor API via get/set
const accessorHandler = (path, target) => ({
    get(_t, prop) {
        switch (prop) {
            case 'value':
                return target === boxedScalarTarget
            ...
            case 'bind':
                return (element, binding, options) => {
            ...
            case 'on': {
                const val = target === boxedScalarTarget
                    ...
                    : target.valueOf
                        ? target.valueOf()
                        : target;
                return (element, eventType) => getOn()(element, eventType, val);
            }
            case 'binding':
                return (binding) => ({
            ...
            case 'listBinding':
                return (content = ({ span }) => span({ bindText: '^' }), options = {}) => {
            ...
            case 'listUpdate':
            case 'listRemove':
                return makeListMethods(path, Array.isArray(target) ? target : [])[prop];
            case 'take':
                return (...args) => {
                    const transform = args[args.length - 1];
                    const extraSources = args.slice(0, -1);
                    const extraPaths = extraSources.map((p) => typeof p === 'string' ? p : p[XIN_PATH]);
                    return {
...
const makeTosiAccessor = (path, target) => new Proxy(target, accessorHandler(path, target));
...
const regHandler = (path, boxScalars) => ({
    get(target, _prop) {
        ...
        if (compoundProp !== null) {
            ...
            return value !== null && typeof value === 'object'
                ? new Proxy(value, regHandler(currentPath, boxScalars))[subPath]
                : value;
        }
        if (prop.startsWith('[') && prop.endsWith(']')) {
        ...
        if ((!Array.isArray(target) && target[prop] !== undefined) ||
            ...
            if (value instanceof Object) {
                ...
                return new Proxy(value instanceof Function ? value.bind(target) : value, regHandler(currentPath, boxScalars));
            }
            else {
            ...
            return typeof value === 'function'
                ? (...items) => {
                    ...
                    if (result != null && typeof result === 'object') {
                        if (prop === 'find' || prop === 'findLast' || prop === 'at') {
                            ...
                            if (index !== -1) {
                                return new Proxy(result, regHandler(extendPath(path, String(index)), boxScalars));
                            }
                        }
                    }
                    return result;
                ...
                : typeof value === 'object'
                    ? new Proxy(tosiValue(value), regHandler(extendPath(path, prop), boxScalars))
                    : boxScalars
    ...
    set(target, prop, value) {
        ...
        const isValueProp = prop === XIN_VALUE ||
            prop === 'xinValue' ||
            prop === 'tosiValue' ||
            (prop === 'value' &&
...
const xin = new Proxy(registry, regHandler('', false));
// Register xin proxy for use by bind.ts (breaks circular dependency)
setXinProxy(xin);
const boxed = new Proxy(registry, regHandler('', true));
