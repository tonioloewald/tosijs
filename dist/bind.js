...
import { elementToBindings, elementToHandlers, LIST_BINDING_REF, BOUND_CLASS, BOUND_SELECTOR, XIN_PATH, XIN_VALUE, TAKE_DESCRIPTOR, } from './metadata';
import { getListItem } from './list-binding';
const { document, MutationObserver } = globalThis;
export const touchElement = (element, changedPath) => {
            ...
            if (path.startsWith('^')) {
                ...
                if (dataSource != null && dataSource[XIN_PATH] != null) {
                    ...
                    if (element instanceof HTMLElement) {
                        console.warn(`Unresolved relative binding "${path}" —`, element, 'is not part of a list. If this is a list template, wrap it in a <template>.');
                    }
                    continue;
                }
            }
        ...
        mutationsList.forEach((mutation) => {
            Array.from(mutation.addedNodes).forEach((node) => {
                if (node instanceof Element) {
                    ...
                    Array.from(node.getElementsByClassName(BOUND_CLASS)).forEach((element) => touchElement(element));
                }
            });
        });
    });
...
observe(() => true, (changedPath) => {
    ...
    const boundElements = Array.from(document.getElementsByClassName(BOUND_CLASS));
    for (const element of boundElements) {
        touchElement(element, changedPath);
    }
});
...
const eventOrigin = (event) => {
    ...
    const origin = event.composed && event.composedPath != null
...
const closestAcrossShadow = (el, selector) => {
    while (el != null) {
...
const nextAcrossShadow = (el, selector) => {
...
const closestHandlerElement = (el) => {
            ...
            if (fromDOM != null) {
                ...
                if (value != null) {
                    ...
                    if (existing == null) {
                        ...
                        const existingActual = existing[XIN_PATH] != null
                            ? existing[XIN_VALUE]
                            : existing;
                        ...
                        if (typeof existingActual === 'number' &&
                            ...
                            if (typeof existingActual === 'number') {
                                ...
                                valueActual = target.value;
                            }
                        }
                        const bothDates = existingActual instanceof Date && valueActual instanceof Date;
                        const changed = bothDates
                            ? existingActual.getTime() !==
                                valueActual.getTime()
...
function bindTake(element, descriptor, binding, options) {
    ...
    const wrappedBinding = {
        toDOM(el, _value, opts) {
            ...
            toDOM(el, result, opts);
        },
        fromDOM: binding.fromDOM,
    };
...
export const warnIfShadowed = (element, what) => {
    if (warnedShadowedBinding)
        return;
    if (typeof ShadowRoot !== 'undefined' &&
        element.getRootNode != null &&
        element.getRootNode() instanceof ShadowRoot) {
        warnedShadowedBinding = true;
        console.warn(`tosijs: ${what} targets an element inside a shadow root, where data ` +
            ...
            'into the shadow DOM. Warned once per session.', element);
    }
};
export function bind(element, what, binding, options) {
    ...
    if (what != null &&
        typeof what === 'object' &&
        what[TAKE_DESCRIPTOR]) {
        return bindTake(element, what, binding, options);
    }
    let path;
    if (typeof what === 'object' &&
        what[XIN_PATH] === undefined &&
        options === undefined) {
        const { value } = what;
    ...
    if (options?.filter && options?.needle) {
        bind(element, options.needle, {
            toDOM(element, value) {
                ;
                element[LIST_BINDING_REF]?.filter(value);
            },
        });
    }
    return element;
...
export function on(element, eventType, eventHandler) {
    ...
    if (!eventBindings[eventType]) {
        eventBindings[eventType] = new Set();
    }
    eventBindings[eventType].add(eventHandler);
    ...
    return () => {
        eventBindings[eventType].delete(eventHandler);
    };
}
