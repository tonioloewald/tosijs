...
const span = globalThis.document !== undefined
...
export class Color {
    ...
    b;
    a;
    static fromVar(varName, element = document.body) {
    ...
    static fromCss(spec) {
        ...
        if (hex) {
            const h = hex[1];
            const p = (i, len) => parseInt(h.slice(i, i + len), 16);
            if (h.length === 3 || h.length === 4) {
                const x = (i) => p(i, 1) * 0x11;
                return new Color(x(0), x(1), x(2), h.length === 4 ? x(3) / 255 : 1);
            }
            if (h.length === 6 || h.length === 8) {
                return new Color(p(0, 2), p(2, 2), p(4, 2), h.length === 8 ? p(6, 2) / 255 : 1);
            }
        }
        ...
        return new Color(Number(r) * scale, Number(g) * scale, Number(b) * scale, a == null ? 1 : Number(a));
    }
    ...
    contrasting(amount = 1) {
        return this.opaque.blend(this.brightness > 0.5 ? Color.black : Color.white, amount);
    }
    get rgb() {
        const { r, g, b } = this;
    ...
    get hsla() {
        const { h, s, l } = this._hsl;
        return `hsl(${h.toFixed(0)}deg ${(s * 100).toFixed(0)}% ${(l * 100).toFixed(0)}% / ${(this.a * 100).toFixed(0)}%)`;
    }
    get mono() {
    ...
    swatch() {
        console.log(`%c      %c ${this.html}, ${this.rgba}`, `background-color: ${this.html}`, 'background-color: transparent');
        return this;
    }
    blend(otherColor, t) {
    ...
    mix(otherColor, t) {
        ...
        return Color.fromHsl(a.s === 0 ? b.h : b.s === 0 ? a.h : Color.blendHue(a.h, b.h, t), lerp(a.s, b.s, t), lerp(a.l, b.l, t), lerp(this.a, otherColor.a, t));
    }
    colorMix(otherColor, t) {
        return Color.fromCss(`color-mix(in hsl, ${this.html}, ${otherColor.html} ${(t * 100).toFixed(0)}%)`);
    }
     ...
     */
    static registerComputedColor(outputVar, sourceVar, scale, method) {
    ...
    static recomputeColors() {
        if (Color.computedColors.size === 0)
            return;
        const rules = [];
        for (const [outputVar, { varName, scale, method },] of Color.computedColors) {
            try {
