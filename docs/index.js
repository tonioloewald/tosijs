...
`,n=n.substring(y.length+1),l=!0),!l){let S=this.rules.other.nextBulletRegex(g),A=this.rules.other.hrRegex(g),j=this.rules.other.fencesBeginRegex(g),z=this.rules.other.headingBeginRegex(g),V=this.rules.other.htmlBeginRegex(g);for(;n;){let q=n.split(`
`,1)[0],F;if(y=q,this.options.pedantic?(y=y.replace(this.rules.other.listReplaceNesting,"  "),F=y):F=y.replace(this.rules.other.tabCharGlobal,"    "),j.test(y)||z.test(y)||V.test(y)||S.test(y)||A.test(y))break;if(F.search(this.rules.other.nonSpaceChar)>=g||!y.trim())c+=`
...
//# debugId=C3630971634D3EC864756E2164756E21
