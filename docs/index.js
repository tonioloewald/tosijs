...
`)?"":`
`)+o.raw,a.text+=`
`+o.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=a.text):t.push(o);continue}if(e){let a="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(a);break}else throw Error(a)}}return this.state.top=!0,t}inline(e,t=[]){return this.inlineQueue.push({src:e,tokens:t}),t}inlineTokens(e,t=[]){let i=e,o=null;if(this.tokens.links){let l=Object.keys(this.tokens.links);if(l.length>0)for(;(o=this.tokenizer.rules.inline.reflinkSearch.exec(i))!=null;)l.includes(o[0].slice(o[0].lastIndexOf("[")+1,-1))&&(i=i.slice(0,o.index)+"["+"a".repeat(o[0].length-2)+"]"+i.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(o=this.tokenizer.rules.inline.anyPunctuation.exec(i))!=null;)i=i.slice(0,o.index)+"++"+i.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let s;for(;(o=this.tokenizer.rules.inline.blockSkip.exec(i))!=null;)s=o[2]?o[2].length:0,i=i.slice(0,o.index+s)+"["+"a".repeat(o[0].length-s-2)+"]"+i.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);i=this.options.hooks?.emStrongMask?.call({lexer:this},i)??i;let a=!1,r="";for(;e;){a||(r=""),a=!1;let l;if(this.options.extensions?.inline?.some((c)=>(l=c.call({lexer:this},e,t))?(e=e.substring(l.raw.length),t.push(l),!0):!1))continue;if(l=this.tokenizer.escape(e)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.tag(e)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.link(e)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.reflink(e,this.tokens.links)){e=e.substring(l.raw.length);let c=t.at(-1);l.type==="text"&&c?.type==="text"?(c.raw+=l.raw,c.text+=l.text):t.push(l);continue}if(l=this.tokenizer.emStrong(e,i,r)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.codespan(e)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.br(e)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.del(e)){e=e.substring(l.raw.length),t.push(l);continue}if(l=this.tokenizer.autolink(e)){e=e.substring(l.raw.length),t.push(l);continue}if(!this.state.inLink&&(l=this.tokenizer.url(e))){e=e.substring(l.raw.length),t.push(l);continue}let h=e;if(this.options.extensions?.startInline){let c=1/0,p=e.slice(1),y;this.options.extensions.startInline.forEach((w)=>{y=w.call({lexer:this},p),typeof y=="number"&&y>=0&&(c=Math.min(c,y))}),c<1/0&&c>=0&&(h=e.substring(0,c+1))}if(l=this.tokenizer.inlineText(h)){e=e.substring(l.raw.length),l.raw.slice(-1)!=="_"&&(r=l.raw.slice(-1)),a=!0;let c=t.at(-1);c?.type==="text"?(c.raw+=l.raw,c.text+=l.text):t.push(l);continue}if(e){let c="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(c);break}else throw Error(c)}}return t}},Wt=class{options;parser;constructor(n){this.options=n||En}space(n){return""}code({text:n,lang:e,escaped:t}){let i=(e||"").match(L.notSpaceStart)?.[0],o=n.replace(L.endingNewline,"")+`
...
\`transport.disconnect()\`.`,title:"A.5 sync",filename:"sync.ts",path:"src/sync.ts"},{text:'# Migrating from `xinjs` to `tosijs`\n\n<!--{ "pin": "bottom" }-->\n\nIn a nutshell:\n\n1. Update to `xinjs` (and `xinjs-ui`) 1.0.6\n2. Fix any issues\n3. Replace all references to "xinjs" with "tosijs"\n\n`xinjs` and `tosijs` 1.0.6 should be identical (likewise `xinjs-ui` and `tosijs-ui`), so the only thing you need to change\nshould be the module names.\n\n> Please [let me know](https://discord.gg/ramJ9rgky5) if there are any issues.\n',title:"Migrating from xinjs to tosijs",filename:"Migration.md",path:"Migration.md",pin:"bottom"}];Yl();var Kn="tosijs";setTimeout(()=>{let n=v.fromVar(d.brandColor),e=v.fromVar(d.background);console.log(`welcome to %c${Kn}`,`background: ${n.html}; color: ${e.html}; padding: 0 5px;`)},100);var Hh=document.location.search!==""?document.location.search.substring(1).split("&")[0]:"README.md",Zh=pi.find((n)=>n.filename===Hh)||pi[0],Gl=at(),{app:X,prefs:Z}=Q({app:{title:Kn,blogUrl:"https://loewald.com",discordUrl:"https://discord.com/invite/ramJ9rgky5",githubUrl:`https://github.com/tonioloewald/${Kn}#readme`,npmUrl:`https://www.npmjs.com/package/${Kn}`,tosijsuiUrl:"https://ui.tosijs.net",bundleBadgeUrl:`https://deno.bundlejs.com/?q=${Kn}&badge=`,bundleUrl:`https://bundlejs.com/?q=${Kn}`,cdnBadgeUrl:`https://data.jsdelivr.com/v1/package/npm/${Kn}/badge`,cdnUrl:`https://www.jsdelivr.com/package/npm/${Kn}`,optimizeLottie:!1,lottieFilename:"",lottieData:"",docs:pi,currentDoc:Zh,compact:!1},prefs:{colorScheme:Gl.colorScheme,highContrast:Gl.highContrast,locale:""}});Zi((n)=>{if(n.startsWith("prefs"))return!0;return!1});on.docLink={toDOM(n,e){n.setAttribute("href",`?${e}`)}};on.current={toDOM(n,e){let t=n.getAttribute("href")||"";n.classList.toggle("current",e===t.substring(1))}};setTimeout(()=>{Object.assign(globalThis,{app:X,tosi:Q,img:yi,bindings:on,elements:x,vars:d,touch:J,Color:v})},1000);var Kl=document.querySelector("main"),{h2:Mh,div:Ql,span:lt,a:ui,img:yi,header:Jh,button:$l,template:Lh,input:Eh}=x;Z.colorScheme.observe(()=>{us({colorScheme:Z.colorScheme.value})});Z.highContrast.observe(()=>{us({highContrast:Z.highContrast.value})});window.addEventListener("popstate",()=>{let n=window.location.search.substring(1);X.currentDoc=X.docs.find((e)=>e.filename===n)||X.docs[0]});var nc=Zn(()=>{console.time("filter");let n=Hl.value.toLocaleLowerCase();X.docs.forEach((e)=>{e.hidden=!e.title.toLocaleLowerCase().includes(n)&&!e.text.toLocaleLowerCase().includes(n)}),J(X.docs),console.timeEnd("filter")}),Hl=Eh({slot:"nav",placeholder:"search",type:"search",style:{width:"calc(100% - 10px)",margin:"5px"},onInput:nc});if(Kl)Kl.append(Jh($l({class:"iconic",style:{color:d.linkColor},title:"navigation",bind:{value:X.compact,binding:{toDOM(n,e){n.style.display=e?"":"none",n.nextSibling.style.display=e?"":"none"}}},onClick(){let n=document.querySelector(ei.tagName);n.contentVisible=!n.contentVisible}},m.menu()),lt({style:{flex:"0 0 10px"}}),ui({href:"/",style:{display:"flex",alignItems:"center",borderBottom:"none"},title:`tosijs ${vt}, tosijs-ui ${hi}`},yi({src:"favicon.svg",style:{height:40,marginRight:10}}),Mh({bindText:"app.title"})),lt({class:"elastic"}),os({minWidth:750},lt({style:{marginRight:d.spacing,display:"flex",alignItems:"center",gap:d.spacing50}},ui({href:X.bundleUrl},yi({alt:"bundlejs size badge",src:X.bundleBadgeUrl})),ui({href:X.cdnUrl},yi({alt:"jsdelivr",src:X.cdnBadgeUrl}))),lt({slot:"small"})),lt({style:{flex:"0 0 10px"}}),$l({title:"theme",class:"iconic",onClick(n){D({target:n.target.closest("button"),menuItems:[{icon:"github",caption:"github",action:X.githubUrl.value},{icon:"npm",caption:"npm",action:X.npmUrl.value},{icon:"discord",caption:"discord",action:X.discordUrl.value},{icon:"tosiUi",caption:"tosijs-ui",action:X.tosijsuiUrl.value},{icon:"blog",caption:"Blog",action:"https://loewald.com"},null,{icon:"rgb",caption:"Color Theme",menuItems:[{caption:"System",checked(){return Z.colorScheme.value==="system"},action(){Z.colorScheme.value="system"}},{caption:"Dark",checked(){return Z.colorScheme.value==="dark"},action(){Z.colorScheme.value="dark"}},{caption:"Light",checked(){return Z.colorScheme.value==="light"},action(){Z.colorScheme.value="light"}},null,{caption:"High Contrast (System)",checked(){return Z.highContrast.value==="system"},action(){Z.highContrast.value="system"}},{caption:"High Contrast On",checked(){return Z.highContrast.value===!0},action(){Z.highContrast.value=!0}},{caption:"High Contrast Off",checked(){return Z.highContrast.value===!1},action(){Z.highContrast.value=!1}}]}]})}},m.moreVertical())),Fo({name:"Documentation",navSize:200,minSize:600,style:{flex:"1 1 auto",overflow:"hidden"},onChange(){let n=document.querySelector(ei.tagName);X.compact.value=n.compact}},Hl,Ql({slot:"nav",style:{display:"flex",flexDirection:"column",width:"100%",height:"100%",overflowY:"scroll"},bindList:{hiddenProp:"hidden",value:X.docs}},Lh(ui({class:"doc-link",bindCurrent:"app.currentDoc.filename",bindDocLink:"^.filename",bindText:"^.title",onClick(n){let e=n.target.closest("a");if(!e)return;let t=un(n.target),i=n.target.closest("xin-sidenav");i.contentVisible=!0;let{href:o}=e;window.history.pushState({href:o},"",o),X.currentDoc=t,n.preventDefault()}}))),Ql({style:{position:"relative",overflowY:"scroll",height:"100%"}},Vo({style:{display:"block",maxWidth:"44em",margin:"auto",padding:"0 1em",overflow:"hidden"},bindValue:"app.currentDoc.text",didRender(){On.insertExamples(this,{tosijs:Re,"tosijs-ui":rs})}}))));

//# debugId=C91B88D932E22CFC64756E2164756E21
