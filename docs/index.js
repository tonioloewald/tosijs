...
`,n=n.substring(g.length+1),l=!0),!l){let T=this.rules.other.nextBulletRegex(w),j=this.rules.other.hrRegex(w),C=this.rules.other.fencesBeginRegex(w),A=this.rules.other.headingBeginRegex(w),Bn=this.rules.other.htmlBeginRegex(w);for(;n;){let Qe=n.split(`
`,1)[0],_n;if(g=Qe,this.options.pedantic?(g=g.replace(this.rules.other.listReplaceNesting,"  "),_n=g):_n=g.replace(this.rules.other.tabCharGlobal,"    "),C.test(g)||A.test(g)||Bn.test(g)||T.test(g)||j.test(g))break;if(_n.search(this.rules.other.nonSpaceChar)>=w||!g.trim())h+=`
