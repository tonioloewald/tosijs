...
\`transport.disconnect()\`.`,title:"A.5 sync",filename:"sync.ts",path:"src/sync.ts"},{text:'# Migrating from `xinjs` to `tosijs`\n\n<!--{ "pin": "bottom" }-->\n\nIn a nutshell:\n\n1. Update to `xinjs` (and `xinjs-ui`) 1.0.6\n2. Fix any issues\n3. Replace all references to "xinjs" with "tosijs"\n\n`xinjs` and `tosijs` 1.0.6 should be identical (likewise `xinjs-ui` and `tosijs-ui`), so the only thing you need to change\nshould be the module names.\n\n> Please [let me know](https://discord.gg/ramJ9rgky5) if there are any issues.\n',title:"Migrating from xinjs to tosijs",filename:"Migration.md",path:"Migration.md",pin:"bottom"}];Yl();var $n="tosijs";setTimeout(()=>{let n=v.fromVar(d.brandColor),e=v.fromVar(d.background);console.log(`welcome to %c${$n}`,`background: ${n.html}; color: ${e.html}; padding: 0 5px;`)},100);var Hh=document.location.search!==""?document.location.search.substring(1).split("&")[0]:"README.md",Zh=yi.find((n)=>n.filename===Hh)||yi[0],Xl=rt(),{app:Y,prefs:J}=H({app:{title:$n,blogUrl:"https://loewald.com",discordUrl:"https://discord.com/invite/ramJ9rgky5",githubUrl:`https://github.com/tonioloewald/${$n}#readme`,npmUrl:`https://www.npmjs.com/package/${$n}`,tosijsuiUrl:"https://ui.tosijs.net",bundleBadgeUrl:`https://deno.bundlejs.com/?q=${$n}&badge=`,bundleUrl:`https://bundlejs.com/?q=${$n}`,cdnBadgeUrl:`https://data.jsdelivr.com/v1/package/npm/${$n}/badge`,cdnUrl:`https://www.jsdelivr.com/package/npm/${$n}`,optimizeLottie:!1,lottieFilename:"",lottieData:"",docs:yi,currentDoc:Zh,compact:!1},prefs:{colorScheme:Xl.colorScheme,highContrast:Xl.highContrast,locale:""}});Ji((n)=>{if(n.startsWith("prefs"))return!0;return!1});sn.docLink={toDOM(n,e){n.setAttribute("href",`?${e}`)}};sn.current={toDOM(n,e){let t=n.getAttribute("href")||"";n.classList.toggle("current",e===t.substring(1))}};setTimeout(()=>{Object.assign(globalThis,{app:Y,tosi:H,img:gi,bindings:sn,elements:x,vars:d,touch:E,Color:v})},1000);var Kl=document.querySelector("main"),{h2:Mh,div:Ql,span:dt,a:mi,img:gi,header:Jh,button:$l,template:Lh,input:Eh}=x;J.colorScheme.observe(()=>{ms({colorScheme:J.colorScheme.value})});J.highContrast.observe(()=>{ms({highContrast:J.highContrast.value})});window.addEventListener("popstate",()=>{let n=window.location.search.substring(1);Y.currentDoc=Y.docs.find((e)=>e.filename===n)||Y.docs[0]});var nc=Jn(()=>{console.time("filter");let n=Hl.value.toLocaleLowerCase();Y.docs.forEach((e)=>{e.hidden=!e.title.toLocaleLowerCase().includes(n)&&!e.text.toLocaleLowerCase().includes(n)}),E(Y.docs),console.timeEnd("filter")}),Hl=Eh({slot:"nav",placeholder:"search",type:"search",style:{width:"calc(100% - 10px)",margin:"5px"},onInput:nc});if(Kl)Kl.append(Jh($l({class:"iconic",style:{color:d.linkColor},title:"navigation",bind:{value:Y.compact,binding:{toDOM(n,e){n.style.display=e?"":"none",n.nextSibling.style.display=e?"":"none"}}},onClick(){let n=document.querySelector(ii.tagName);n.contentVisible=!n.contentVisible}},m.menu()),dt({style:{flex:"0 0 10px"}}),mi({href:"/",style:{display:"flex",alignItems:"center",borderBottom:"none"},title:`tosijs ${Ct}, tosijs-ui ${pi}`},gi({src:"favicon.svg",style:{height:40,marginRight:10}}),Mh({bindText:"app.title"})),dt({class:"elastic"}),as({minWidth:750},dt({style:{marginRight:d.spacing,display:"flex",alignItems:"center",gap:d.spacing50}},mi({href:Y.bundleUrl},gi({alt:"bundlejs size badge",src:Y.bundleBadgeUrl})),mi({href:Y.cdnUrl},gi({alt:"jsdelivr",src:Y.cdnBadgeUrl}))),dt({slot:"small"})),dt({style:{flex:"0 0 10px"}}),$l({title:"theme",class:"iconic",onClick(n){R({target:n.target.closest("button"),menuItems:[{icon:"github",caption:"github",action:Y.githubUrl.value},{icon:"npm",caption:"npm",action:Y.npmUrl.value},{icon:"discord",caption:"discord",action:Y.discordUrl.value},{icon:"tosiUi",caption:"tosijs-ui",action:Y.tosijsuiUrl.value},{icon:"blog",caption:"Blog",action:"https://loewald.com"},null,{icon:"rgb",caption:"Color Theme",menuItems:[{caption:"System",checked(){return J.colorScheme.value==="system"},action(){J.colorScheme.value="system"}},{caption:"Dark",checked(){return J.colorScheme.value==="dark"},action(){J.colorScheme.value="dark"}},{caption:"Light",checked(){return J.colorScheme.value==="light"},action(){J.colorScheme.value="light"}},null,{caption:"High Contrast (System)",checked(){return J.highContrast.value==="system"},action(){J.highContrast.value="system"}},{caption:"High Contrast On",checked(){return J.highContrast.value===!0},action(){J.highContrast.value=!0}},{caption:"High Contrast Off",checked(){return J.highContrast.value===!1},action(){J.highContrast.value=!1}}]}]})}},m.moreVertical())),Yo({name:"Documentation",navSize:200,minSize:600,style:{flex:"1 1 auto",overflow:"hidden"},onChange(){let n=document.querySelector(ii.tagName);Y.compact.value=n.compact}},Hl,Ql({slot:"nav",style:{display:"flex",flexDirection:"column",width:"100%",height:"100%",overflowY:"scroll"},bindList:{hiddenProp:"hidden",value:Y.docs}},Lh(mi({class:"doc-link",bindCurrent:"app.currentDoc.filename",bindDocLink:"^.filename",bindText:"^.title",onClick(n){let e=n.target.closest("a");if(!e)return;let t=mn(n.target),i=n.target.closest("xin-sidenav");i.contentVisible=!0;let{href:o}=e;window.history.pushState({href:o},"",o),Y.currentDoc=t,n.preventDefault()}}))),Ql({style:{position:"relative",overflowY:"scroll",height:"100%"}},_o({style:{display:"block",maxWidth:"44em",margin:"auto",padding:"0 1em",overflow:"hidden"},bindValue:"app.currentDoc.text",didRender(){In.insertExamples(this,{tosijs:We,"tosijs-ui":hs})}}))));

//# debugId=A940318BD412827664756E2164756E21
