...
`).split(`
`),a="",i="",o=[];for(;t.length>0;){let s=!1,r=[],l;for(l=0;l<t.length;l++)if(this.rules.other.blockquoteStart.test(t[l]))r.push(t[l]),s=!0;else if(!s)r.push(t[l]);else break;t=t.slice(l);let c=r.join(`
`),h=c.replace(this.rules.other.blockquoteSetextReplace,`
    $1`).replace(this.rules.other.blockquoteSetextReplace2,"");a=a?`${a}
${c}`:c,i=i?`${i}
${h}`:h;let u=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(h,o,!0),this.lexer.state.top=u,t.length===0)break;let b=o.at(-1);if(b?.type==="code")break;if(b?.type==="blockquote"){let f=b,w=f.raw+`
`+t.join(`
...
`);continue}}return{type:"blockquote",raw:a,tokens:o,text:i}}}list(n){let e=this.rules.block.list.exec(n);if(e){let t=e[1].trim(),a=t.length>1,i={type:"list",raw:"",ordered:a,start:a?+t.slice(0,-1):"",loose:!1,items:[]};t=a?`\\d{1,9}\\${t.slice(-1)}`:`\\${t}`,this.options.pedantic&&(t=a?t:"[*+-]");let o=this.rules.other.listItemRegex(t),s=!1;for(;n;){let l=!1,c="",h="";if(!(e=o.exec(n))||this.rules.block.hr.test(n))break;c=e[0],n=n.substring(c.length);let u=e[2].split(`
`,1)[0].replace(this.rules.other.listReplaceTabs,(P)=>" ".repeat(3*P.length)),b=n.split(`
`,1)[0],f=!u.trim(),w=0;if(this.options.pedantic?(w=2,h=u.trimStart()):f?w=e[1].length+1:(w=e[2].search(this.rules.other.nonSpaceChar),w=w>4?1:w,h=u.slice(w),w+=e[1].length),f&&this.rules.other.blankLine.test(b)&&(c+=b+`
`,n=n.substring(b.length+1),l=!0),!l){let P=this.rules.other.nextBulletRegex(w),x=this.rules.other.hrRegex(w),j=this.rules.other.fencesBeginRegex(w),S=this.rules.other.headingBeginRegex(w),R=this.rules.other.htmlBeginRegex(w);for(;n;){let mn=n.split(`
`,1)[0],an;if(b=mn,this.options.pedantic?(b=b.replace(this.rules.other.listReplaceNesting,"  "),an=b):an=b.replace(this.rules.other.tabCharGlobal,"    "),j.test(b)||S.test(b)||R.test(b)||P.test(b)||x.test(b))break;if(an.search(this.rules.other.nonSpaceChar)>=w||!b.trim())h+=`
`+an.slice(w);else{if(f||u.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||j.test(u)||S.test(u)||x.test(u))break;h+=`
`+b}!f&&!b.trim()&&(f=!0),c+=mn+`
`,n=n.substring(mn.length+1),u=an.slice(w)}}i.loose||(s?i.loose=!0:this.rules.other.doubleBlankLine.test(c)&&(s=!0));let k=null,I;this.options.gfm&&(k=this.rules.other.listIsTask.exec(h),k&&(I=k[0]!=="[ ] ",h=h.replace(this.rules.other.listReplaceTask,""))),i.items.push({type:"list_item",raw:c,task:!!k,checked:I,loose:!1,text:h,tokens:[]}),i.raw+=c}let r=i.items.at(-1);if(r)r.raw=r.raw.trimEnd(),r.text=r.text.trimEnd();else return;i.raw=i.raw.trimEnd();for(let l=0;l<i.items.length;l++)if(this.lexer.state.top=!1,i.items[l].tokens=this.lexer.blockTokens(i.items[l].text,[]),!i.loose){let c=i.items[l].tokens.filter((u)=>u.type==="space"),h=c.length>0&&c.some((u)=>this.rules.other.anyLine.test(u.raw));i.loose=h}if(i.loose)for(let l=0;l<i.items.length;l++)i.items[l].loose=!0;return i}}html(n){let e=this.rules.block.html.exec(n);if(e)return{type:"html",block:!0,raw:e[0],pre:e[1]==="pre"||e[1]==="script"||e[1]==="style",text:e[0]}}def(n){let e=this.rules.block.def.exec(n);if(e){let t=e[1].toLowerCase().replace(this.rules.other.multipleSpaceGlobal," "),a=e[2]?e[2].replace(this.rules.other.hrefBrackets,"$1").replace(this.rules.inline.anyPunctuation,"$1"):"",i=e[3]?e[3].substring(1,e[3].length-1).replace(this.rules.inline.anyPunctuation,"$1"):e[3];return{type:"def",tag:t,raw:e[0],href:a,title:i}}}table(n){let e=this.rules.block.table.exec(n);if(!e||!this.rules.other.tableDelimiter.test(e[2]))return;let t=Ni(e[1]),a=e[2].replace(this.rules.other.tableAlignChars,"").split("|"),i=e[3]?.trim()?e[3].replace(this.rules.other.tableRowBlankLine,"").split(`
...
`,title:"todo",filename:"TODO.md",path:"TODO.md",pin:"bottom"}];Yo();var un="tosijs";setTimeout(()=>{let n=y.fromVar(d.brandColor),e=y.fromVar(d.background);console.log(`welcome to %c${un}`,`background: ${n.html}; color: ${e.html}; padding: 0 5px;`)},100);var fd=document.location.search!==""?document.location.search.substring(1).split("&")[0]:"README.md",xd=et.find((n)=>n.filename===fd)||et[0],Jo=ye(),{app:B,prefs:A}=W({app:{title:un,blogUrl:"https://loewald.com",discordUrl:"https://discord.com/invite/ramJ9rgky5",githubUrl:`https://github.com/tonioloewald/${un}#readme`,npmUrl:`https://www.npmjs.com/package/${un}`,tosijsuiUrl:"https://ui.tosijs.net",bundleBadgeUrl:`https://deno.bundlejs.com/?q=${un}&badge=`,bundleUrl:`https://bundlejs.com/?q=${un}`,cdnBadgeUrl:`https://data.jsdelivr.com/v1/package/npm/${un}/badge`,cdnUrl:`https://www.jsdelivr.com/package/npm/${un}`,optimizeLottie:!1,lottieFilename:"",lottieData:"",docs:et,currentDoc:xd,compact:!1},prefs:{colorScheme:Jo.colorScheme,highContrast:Jo.highContrast,locale:""}});Ot((n)=>{if(n.startsWith("prefs"))return!0;return!1});en.docLink={toDOM(n,e){n.setAttribute("href",`?${e}`)}};en.current={toDOM(n,e){let t=n.getAttribute("href")||"";n.classList.toggle("current",e===t.substring(1))}};setTimeout(()=>{Object.assign(globalThis,{app:B,tosi:W,img:at,bindings:en,elements:m,vars:d,touch:nn,Color:y})},1000);var Go=document.querySelector("main"),{h2:kd,div:$o,span:ve,a:tt,img:at,header:jd,button:Ko,template:Sd,input:Cd}=m;A.colorScheme.observe(()=>{Ma({colorScheme:A.colorScheme.value})});A.highContrast.observe(()=>{Ma({highContrast:A.highContrast.value})});window.addEventListener("popstate",()=>{let n=window.location.search.substring(1);B.currentDoc=B.docs.find((e)=>e.filename===n)||B.docs[0]});var Td=Ie(()=>{console.time("filter");let n=Zo.value.toLocaleLowerCase();B.docs.forEach((e)=>{e.hidden=!e.title.toLocaleLowerCase().includes(n)&&!e.text.toLocaleLowerCase().includes(n)}),nn(B.docs),console.timeEnd("filter")}),Zo=Cd({slot:"nav",placeholder:"search",type:"search",style:{width:"calc(100% - 10px)",margin:"5px"},onInput:Td});if(Go)Go.append(jd(Ko({class:"iconic",style:{color:d.linkColor},title:"navigation",bind:{value:B.compact,binding:{toDOM(n,e){n.style.display=e?"":"none",n.nextSibling.style.display=e?"":"none"}}},onClick(){let n=document.querySelector(Jn.tagName);n.contentVisible=!n.contentVisible}},v.menu()),ve({style:{flex:"0 0 10px"}}),tt({href:"/",style:{display:"flex",alignItems:"center",borderBottom:"none"},title:`tosijs ${Oe}, tosijs-ui ${za}`},at({src:"favicon.svg",style:{height:40,marginRight:10}}),kd({bindText:"app.title"})),ve({class:"elastic"}),Aa({minWidth:750},ve({style:{marginRight:d.spacing,display:"flex",alignItems:"center",gap:d.spacing50}},tt({href:B.bundleUrl},at({alt:"bundlejs size badge",src:B.bundleBadgeUrl})),tt({href:B.cdnUrl},at({alt:"jsdelivr",src:B.cdnBadgeUrl}))),ve({slot:"small"})),ve({style:{flex:"0 0 10px"}}),Ko({title:"theme",class:"iconic",onClick(n){Q({target:n.target.closest("button"),menuItems:[{icon:"github",caption:"github",action:B.githubUrl.value},{icon:"npm",caption:"npm",action:B.npmUrl.value},{icon:"discord",caption:"discord",action:B.discordUrl.value},{icon:"tosiUi",caption:"tosijs-ui",action:B.tosijsuiUrl.value},{icon:"blog",caption:"Blog",action:"https://loewald.com"},null,{icon:"rgb",caption:"Color Theme",menuItems:[{caption:"System",checked(){return A.colorScheme.value==="system"},action(){A.colorScheme.value="system"}},{caption:"Dark",checked(){return A.colorScheme.value==="dark"},action(){A.colorScheme.value="dark"}},{caption:"Light",checked(){return A.colorScheme.value==="light"},action(){A.colorScheme.value="light"}},null,{caption:"High Contrast (System)",checked(){return A.highContrast.value==="system"},action(){A.highContrast.value="system"}},{caption:"High Contrast On",checked(){return A.highContrast.value===!0},action(){A.highContrast.value=!0}},{caption:"High Contrast Off",checked(){return A.highContrast.value===!1},action(){A.highContrast.value=!1}}]}]})}},v.moreVertical())),Oa({name:"Documentation",navSize:200,minSize:600,style:{flex:"1 1 auto",overflow:"hidden"},onChange(){let n=document.querySelector(Jn.tagName);B.compact.value=n.compact}},Zo,$o({slot:"nav",style:{display:"flex",flexDirection:"column",width:"100%",height:"100%",overflowY:"scroll"},bindList:{hiddenProp:"hidden",value:B.docs}},Sd(tt({class:"doc-link",bindCurrent:"app.currentDoc.filename",bindDocLink:"^.filename",bindText:"^.title",onClick(n){let e=n.target.closest("a");if(!e)return;let t=kn(n.target),a=n.target.closest("xin-sidenav");a.contentVisible=!0;let{href:i}=e;window.history.pushState({href:i},"",i),B.currentDoc=t,n.preventDefault()}}))),$o({style:{position:"relative",overflowY:"scroll",height:"100%"}},ja({style:{display:"block",maxWidth:"44em",margin:"auto",padding:"0 1em",overflow:"hidden"},bindValue:"app.currentDoc.text",didRender(){Yn.insertExamples(this,{tosijs:oe,"tosijs-ui":Ra})}}))));

//# debugId=D1720BD16864A86164756E2164756E21
