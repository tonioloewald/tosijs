...
`):e}}}fences(n){let t=this.rules.block.fences.exec(n);if(t){let e=t[0],a=Hi(e,t[3]||"");return{type:"code",raw:e,lang:t[2]?t[2].trim().replace(this.rules.inline._escapes,"$1"):t[2],text:a}}}heading(n){let t=this.rules.block.heading.exec(n);if(t){let e=t[2].trim();if(/#$/.test(e)){let a=mt(e,"#");if(this.options.pedantic)e=a.trim();else if(!a||/ $/.test(a))e=a.trim()}return{type:"heading",raw:t[0],depth:t[1].length,text:e,tokens:this.lexer.inline(e)}}}hr(n){let t=this.rules.block.hr.exec(n);if(t)return{type:"hr",raw:t[0]}}blockquote(n){let t=this.rules.block.blockquote.exec(n);if(t){let e=t[0].replace(/^ *>[ \t]?/gm,""),a=this.lexer.state.top;this.lexer.state.top=!0;let o=this.lexer.blockTokens(e);return this.lexer.state.top=a,{type:"blockquote",raw:t[0],tokens:o,text:e}}}list(n){let t=this.rules.block.list.exec(n);if(t){let e,a,o,i,s,l,d,h,c,u,p,j,B=t[1].trim(),H=B.length>1,T={type:"list",raw:"",ordered:H,start:H?+B.slice(0,-1):"",loose:!1,items:[]};if(B=H?`\\d{1,9}\\${B.slice(-1)}`:`\\${B}`,this.options.pedantic)B=H?B:"[*+-]";let x=new RegExp(`^( {0,3}${B})((?:[	 ][^\\n]*)?(?:\\n|$))`);while(n){if(j=!1,!(t=x.exec(n)))break;if(this.rules.block.hr.test(n))break;if(e=t[0],n=n.substring(e.length),h=t[2].split(`
`,1)[0].replace(/^\t+/,(E)=>" ".repeat(3*E.length)),c=n.split(`
`,1)[0],this.options.pedantic)i=2,p=h.trimLeft();else i=t[2].search(/[^ ]/),i=i>4?1:i,p=h.slice(i),i+=t[1].length;if(l=!1,!h&&/^ *$/.test(c))e+=c+`
`,n=n.substring(c.length+1),j=!0;if(!j){let E=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),z=new RegExp(`^ {0,${Math.min(3,i-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),N=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:\`\`\`|~~~)`),An=new RegExp(`^ {0,${Math.min(3,i-1)}}#`);while(n){if(u=n.split(`
`,1)[0],c=u,this.options.pedantic)c=c.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ");if(N.test(c))break;if(An.test(c))break;if(E.test(c))break;if(z.test(n))break;if(c.search(/[^ ]/)>=i||!c.trim())p+=`
`+c.slice(i);else{if(l)break;if(h.search(/[^ ]/)>=4)break;if(N.test(h))break;if(An.test(h))break;if(z.test(h))break;p+=`
`+c}if(!l&&!c.trim())l=!0;e+=u+`
`,n=n.substring(u.length+1),h=c.slice(i)}}if(!T.loose){if(d)T.loose=!0;else if(/\n *\n *$/.test(e))d=!0}if(this.options.gfm){if(a=/^\[[ xX]\] /.exec(p),a)o=a[0]!=="[ ] ",p=p.replace(/^\[[ xX]\] +/,"")}T.items.push({type:"list_item",raw:e,task:!!a,checked:o,loose:!1,text:p}),T.raw+=e}T.items[T.items.length-1].raw=e.trimRight(),T.items[T.items.length-1].text=p.trimRight(),T.raw=T.raw.trimRight();let _=T.items.length;for(s=0;s<_;s++)if(this.lexer.state.top=!1,T.items[s].tokens=this.lexer.blockTokens(T.items[s].text,[]),!T.loose){let E=T.items[s].tokens.filter((N)=>N.type==="space"),z=E.length>0&&E.some((N)=>/\n.*\n/.test(N.raw));T.loose=z}if(T.loose)for(s=0;s<_;s++)T.items[s].loose=!0;return T}}html(n){let t=this.rules.block.html.exec(n);if(t){let e={type:"html",block:!0,raw:t[0],pre:!this.options.sanitizer&&(t[1]==="pre"||t[1]==="script"||t[1]==="style"),text:t[0]};if(this.options.sanitize){let a=this.options.sanitizer?this.options.sanitizer(t[0]):D(t[0]);e.type="paragraph",e.text=a,e.tokens=this.lexer.inline(a)}return e}}def(n){let t=this.rules.block.def.exec(n);if(t){let e=t[1].toLowerCase().replace(/\s+/g," "),a=t[2]?t[2].replace(/^<(.*)>$/,"$1").replace(this.rules.inline._escapes,"$1"):"",o=t[3]?t[3].substring(1,t[3].length-1).replace(this.rules.inline._escapes,"$1"):t[3];return{type:"def",tag:e,raw:t[0],href:a,title:o}}}table(n){let t=this.rules.block.table.exec(n);if(t){let e={type:"table",header:Ca(t[1]).map((a)=>{return{text:a}}),align:t[2].replace(/^ *|\| *$/g,"").split(/ *\| */),rows:t[3]&&t[3].trim()?t[3].replace(/\n[ \t]*$/,"").split(`
...
`,title:"todo",filename:"TODO.md",path:"TODO.md",pin:"bottom"}];Ln("demo-style",Io);var sn="tosijs";setTimeout(()=>{let n=v.fromVar(r.brandColor),t=v.fromVar(r.background);console.log(`welcome to %c${sn}`,`background: ${n.html}; color: ${t.html}; padding: 0 5px;`)},100);var ml=document.location.search!==""?document.location.search.substring(1).split("&")[0]:"README.md",cl=_t.find((n)=>n.filename===ml)||_t[0],{app:A,prefs:V}=$n({app:{title:sn,blogUrl:"https://loewald.com",discordUrl:"https://discord.com/invite/ramJ9rgky5",githubUrl:`https://github.com/tonioloewald/${sn}#readme`,npmUrl:`https://www.npmjs.com/package/${sn}`,xinjsuiUrl:"https://ui.xinjs.net",bundleBadgeUrl:`https://deno.bundlejs.com/?q=${sn}&badge=`,bundleUrl:`https://bundlejs.com/?q=${sn}`,cdnBadgeUrl:`https://data.jsdelivr.com/v1/package/npm/${sn}/badge`,cdnUrl:`https://www.jsdelivr.com/package/npm/${sn}`,optimizeLottie:!1,lottieFilename:"",lottieData:"",docs:_t,currentDoc:cl},prefs:{theme:"system",highContrast:!1,monochrome:!1,locale:""}});Kt((n)=>{if(n.startsWith("prefs"))return!0;return!1});K.docLink={toDOM(n,t){n.setAttribute("href",`?${t}`)}};K.current={toDOM(n,t){let e=n.getAttribute("href")||"";n.classList.toggle("current",t===e.substring(1))}};setTimeout(()=>{Object.assign(globalThis,{app:A,tosi:$n,img:zt,bindings:K,elements:g,vars:r,touch:un,Color:v})},1000);var Po=document.querySelector("main"),{h2:bl,div:Mo,span:Dt,a:At,img:zt,header:gl,button:Bo,template:yl,input:fl}=g;X(document.body,"prefs.theme",{toDOM(n,t){if(t==="system")t=getComputedStyle(document.body).getPropertyValue("--darkmode")==="true"?"dark":"light";n.classList.toggle("darkmode",t==="dark")}});X(document.body,V.highContrast,{toDOM(n,t){n.classList.toggle("high-contrast",t.valueOf())}});X(document.body,V.monochrome,{toDOM(n,t){n.classList.toggle("monochrome",t.valueOf())}});window.addEventListener("popstate",()=>{let n=window.location.search.substring(1);A.currentDoc=A.docs.find((t)=>t.filename===n)||A.docs[0]});var wl=lt(()=>{console.time("filter");let n=_o.value.toLocaleLowerCase();A.docs.forEach((t)=>{t.hidden=!t.title.toLocaleLowerCase().includes(n)&&!t.text.toLocaleLowerCase().includes(n)}),un(A.docs),console.timeEnd("filter")}),_o=fl({slot:"nav",placeholder:"search",type:"search",style:{width:"calc(100% - 10px)",margin:"5px"},onInput:wl});if(Po)Po.append(gl(At({href:"/",style:{display:"flex",alignItems:"center",borderBottom:"none"},title:`tosijs ${ht}, tosijs-ui ${Xe}`},zt({src:"favicon.svg",style:{height:40,marginRight:10}}),bl({bindText:"app.title"})),Dt({class:"elastic"}),Ve({minWidth:750},Dt({style:{marginRight:r.spacing,display:"flex",alignItems:"center",gap:r.spacing50}},At({href:A.bundleUrl},zt({alt:"bundlejs size badge",src:A.bundleBadgeUrl})),At({href:A.cdnUrl},zt({alt:"jsdelivr",src:A.cdnBadgeUrl}))),Dt({slot:"small"})),Dt({style:{flex:"0 0 10px"}}),Bo({title:"theme",class:"iconic",onClick(n){Q({target:n.target,menuItems:[{icon:"github",caption:"github",action:A.githubUrl},{icon:"npm",caption:"npm",action:A.npmUrl},{icon:"discord",caption:"discord",action:A.discordUrl},{icon:"xinjsUiColor",caption:"xinjs-ui",action:A.xinjsuiUrl},{icon:"blog",caption:"Blog",action:"https://loewald.com"},null,{icon:"rgb",caption:"Color Theme",menuItems:[{caption:"System",checked(){return V.theme==="system"},action(){V.theme="system"}},{caption:"Dark",checked(){return V.theme==="dark"},action(){V.theme="dark"}},{caption:"Light",checked(){return V.theme==="light"},action(){V.theme="light"}},null,{caption:"High Contrast",checked(){return V.highContrast.valueOf()},action(){V.highContrast=!V.highContrast.valueOf()}},{caption:"Monochrome",checked(){return V.monochrome.valueOf()},action(){V.monochrome=!V.monochrome.valueOf()}}]}]})}},f.moreVertical())),ze({name:"Documentation",navSize:200,minSize:600,style:{flex:"1 1 auto",overflow:"hidden"}},_o,Mo({slot:"nav",style:{display:"flex",flexDirection:"column",width:"100%",height:"100%",overflowY:"scroll"},bindList:{hiddenProp:"hidden",value:A.docs}},yl(At({class:"doc-link",bindCurrent:"app.currentDoc.filename",bindDocLink:"^.filename",bindText:"^.title",onClick(n){let t=n.target.closest("a");if(!t)return;let e=pn(n.target),a=n.target.closest("xin-sidenav");a.contentVisible=!0;let{href:o}=t;window.history.pushState({href:o},"",o),A.currentDoc=e,n.preventDefault()}}))),Mo({style:{position:"relative",overflowY:"scroll",height:"100%"}},Bo({title:"show navigation",class:"transparent close-nav show-within-compact",style:{marginTop:"2px",position:"fixed"},onClick(n){n.target.closest("xin-sidenav").contentVisible=!1}},f.chevronLeft()),Ie({style:{display:"block",maxWidth:"44em",margin:"auto",padding:"0 1em",overflow:"hidden"},bindValue:"app.currentDoc.text",didRender(){Dn.insertExamples(this,{tosijs:Gn,tosijsui:qe})}}))));

//# debugId=60E214F0DC6E73D464756E2164756E21
