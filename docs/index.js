...
`,n=n.substring(g.length+1),d=!0),!d){let T=this.rules.other.nextBulletRegex(w),j=this.rules.other.hrRegex(w),C=this.rules.other.fencesBeginRegex(w),B=this.rules.other.headingBeginRegex(w),kn=this.rules.other.htmlBeginRegex(w);for(;n;){let qe=n.split(`
`,1)[0],An;if(g=qe,this.options.pedantic?(g=g.replace(this.rules.other.listReplaceNesting,"  "),An=g):An=g.replace(this.rules.other.tabCharGlobal,"    "),C.test(g)||B.test(g)||kn.test(g)||T.test(g)||j.test(g))break;if(An.search(this.rules.other.nonSpaceChar)>=w||!g.trim())p+=`
...
//# debugId=9FE7B6923C7E72C964756E2164756E21
