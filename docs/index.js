...
`,n=n.substring(c.length+1),j=!0;if(!j){let E=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:[*+-]|\\d{1,9}[.)])((?:[ 	][^\\n]*)?(?:\\n|$))`),O=new RegExp(`^ {0,${Math.min(3,i-1)}}((?:- *){3,}|(?:_ *){3,}|(?:\\* *){3,})(?:\\n+|$)`),H=new RegExp(`^ {0,${Math.min(3,i-1)}}(?:\`\`\`|~~~)`),An=new RegExp(`^ {0,${Math.min(3,i-1)}}#`);while(n){if(u=n.split(`
`,1)[0],c=u,this.options.pedantic)c=c.replace(/^ {1,4}(?=( {4})*[^ ])/g,"  ");if(H.test(c))break;if(An.test(c))break;if(E.test(c))break;if(O.test(n))break;if(c.search(/[^ ]/)>=i||!c.trim())p+=`
`+c.slice(i);else{if(l)break;if(h.search(/[^ ]/)>=4)break;if(H.test(h))break;if(An.test(h))break;if(O.test(h))break;p+=`
`+c}if(!l&&!c.trim())l=!0;e+=u+`
`,n=n.substring(u.length+1),h=c.slice(i)}}if(!T.loose){if(d)T.loose=!0;else if(/\n *\n *$/.test(e))d=!0}if(this.options.gfm){if(a=/^\[[ xX]\] /.exec(p),a)o=a[0]!=="[ ] ",p=p.replace(/^\[[ xX]\] +/,"")}T.items.push({type:"list_item",raw:e,task:!!a,checked:o,loose:!1,text:p}),T.raw+=e}T.items[T.items.length-1].raw=e.trimRight(),T.items[T.items.length-1].text=p.trimRight(),T.raw=T.raw.trimRight();let B=T.items.length;for(s=0;s<B;s++)if(this.lexer.state.top=!1,T.items[s].tokens=this.lexer.blockTokens(T.items[s].text,[]),!T.loose){let E=T.items[s].tokens.filter((H)=>H.type==="space"),O=E.length>0&&E.some((H)=>/\n.*\n/.test(H.raw));T.loose=O}if(T.loose)for(s=0;s<B;s++)T.items[s].loose=!0;return T}}html(n){let t=this.rules.block.html.exec(n);if(t){let e={type:"html",block:!0,raw:t[0],pre:!this.options.sanitizer&&(t[1]==="pre"||t[1]==="script"||t[1]==="style"),text:t[0]};if(this.options.sanitize){let a=this.options.sanitizer?this.options.sanitizer(t[0]):D(t[0]);e.type="paragraph",e.text=a,e.tokens=this.lexer.inline(a)}return e}}def(n){let t=this.rules.block.def.exec(n);if(t){let e=t[1].toLowerCase().replace(/\s+/g," "),a=t[2]?t[2].replace(/^<(.*)>$/,"$1").replace(this.rules.inline._escapes,"$1"):"",o=t[3]?t[3].substring(1,t[3].length-1).replace(this.rules.inline._escapes,"$1"):t[3];return{type:"def",tag:e,raw:t[0],href:a,title:o}}}table(n){let t=this.rules.block.table.exec(n);if(t){let e={type:"table",header:Ca(t[1]).map((a)=>{return{text:a}}),align:t[2].replace(/^ *|\| *$/g,"").split(/ *\| */),rows:t[3]&&t[3].trim()?t[3].replace(/\n[ \t]*$/,"").split(`
...
`}strong(n){return`<strong>${n}</strong>`}em(n){return`<em>${n}</em>`}codespan(n){return`<code>${n}</code>`}br(){return this.options.xhtml?"<br/>":"<br>"}del(n){return`<del>${n}</del>`}link(n,t,e){if(n=xa(this.options.sanitize,this.options.baseUrl,n),n===null)return e;let a='<a href="'+n+'"';if(t)a+=' title="'+t+'"';return a+=">"+e+"</a>",a}image(n,t,e){if(n=xa(this.options.sanitize,this.options.baseUrl,n),n===null)return e;let a=`<img src="${n}" alt="${e}"`;if(t)a+=` title="${t}"`;return a+=this.options.xhtml?"/>":">",a}text(n){return n}}class bt{strong(n){return n}em(n){return n}codespan(n){return n}del(n){return n}html(n){return n}text(n){return n}link(n,t,e){return""+e}image(n,t,e){return""+e}br(){return""}}class gt{constructor(){this.seen={}}serialize(n){return n.toLowerCase().trim().replace(/<[!\/a-z].*?>/ig,"").replace(/[\u2000-\u206F\u2E00-\u2E7F\\'!"#$%&()*+,./:;<=>?@[\]^`{|}~]/g,"").replace(/\s/g,"-")}getNextSafeSlug(n,t){let e=n,a=0;if(this.seen.hasOwnProperty(e)){a=this.seen[n];do a++,e=n+"-"+a;while(this.seen.hasOwnProperty(e))}if(!t)this.seen[n]=a,this.seen[e]=0;return e}slug(n,t={}){let e=this.serialize(n);return this.getNextSafeSlug(e,t.dryrun)}}class G{constructor(n){this.options=n||nn,this.options.renderer=this.options.renderer||new Rn,this.renderer=this.options.renderer,this.renderer.options=this.options,this.textRenderer=new bt,this.slugger=new gt}static parse(n,t){return new G(t).parse(n)}static parseInline(n,t){return new G(t).parseInline(n)}parse(n,t=!0){let e="",a,o,i,s,l,d,h,c,u,p,j,_,V,T,x,B,E,O,H,An=n.length;for(a=0;a<An;a++){if(p=n[a],this.options.extensions&&this.options.extensions.renderers&&this.options.extensions.renderers[p.type]){if(H=this.options.extensions.renderers[p.type].call({parser:this},p),H!==!1||!["space","hr","heading","code","table","blockquote","list","html","paragraph","text"].includes(p.type)){e+=H||"";continue}}switch(p.type){case"space":continue;case"hr":{e+=this.renderer.hr();continue}case"heading":{e+=this.renderer.heading(this.parseInline(p.tokens),p.depth,Pa(this.parseInline(p.tokens,this.textRenderer)),this.slugger);continue}case"code":{e+=this.renderer.code(p.text,p.lang,p.escaped);continue}case"table":{c="",h="",s=p.header.length;for(o=0;o<s;o++)h+=this.renderer.tablecell(this.parseInline(p.header[o].tokens),{header:!0,align:p.align[o]});c+=this.renderer.tablerow(h),u="",s=p.rows.length;for(o=0;o<s;o++){d=p.rows[o],h="",l=d.length;for(i=0;i<l;i++)h+=this.renderer.tablecell(this.parseInline(d[i].tokens),{header:!1,align:p.align[i]});u+=this.renderer.tablerow(h)}e+=this.renderer.table(c,u);continue}case"blockquote":{u=this.parse(p.tokens),e+=this.renderer.blockquote(u);continue}case"list":{j=p.ordered,_=p.start,V=p.loose,s=p.items.length,u="";for(o=0;o<s;o++){if(x=p.items[o],B=x.checked,E=x.task,T="",x.task)if(O=this.renderer.checkbox(B),V)if(x.tokens.length>0&&x.tokens[0].type==="paragraph"){if(x.tokens[0].text=O+" "+x.tokens[0].text,x.tokens[0].tokens&&x.tokens[0].tokens.length>0&&x.tokens[0].tokens[0].type==="text")x.tokens[0].tokens[0].text=O+" "+x.tokens[0].tokens[0].text}else x.tokens.unshift({type:"text",text:O});else T+=O;T+=this.parse(x.tokens,V),u+=this.renderer.listitem(T,E,B)}e+=this.renderer.list(u,j,_);continue}case"html":{e+=this.renderer.html(p.text,p.block);continue}case"paragraph":{e+=this.renderer.paragraph(this.parseInline(p.tokens));continue}case"text":{u=p.tokens?this.parseInline(p.tokens):p.text;while(a+1<An&&n[a+1].type==="text")p=n[++a],u+=`
...
//# debugId=E88BCC52F9B6FED364756E2164756E21
