...
`,n=n.substring(b.length+1),l=!0),!l){let P=this.rules.other.nextBulletRegex(w),x=this.rules.other.hrRegex(w),j=this.rules.other.fencesBeginRegex(w),S=this.rules.other.headingBeginRegex(w),z=this.rules.other.htmlBeginRegex(w);for(;n;){let mn=n.split(`
`,1)[0],an;if(b=mn,this.options.pedantic?(b=b.replace(this.rules.other.listReplaceNesting,"  "),an=b):an=b.replace(this.rules.other.tabCharGlobal,"    "),j.test(b)||S.test(b)||z.test(b)||P.test(b)||x.test(b))break;if(an.search(this.rules.other.nonSpaceChar)>=w||!b.trim())h+=`
