...
`,n=n.substring(y.length+1),l=!0),!l){let S=this.rules.other.nextBulletRegex(g),j=this.rules.other.hrRegex(g),A=this.rules.other.fencesBeginRegex(g),I=this.rules.other.headingBeginRegex(g),tn=this.rules.other.htmlBeginRegex(g);for(;n;){let W=n.split(`
`,1)[0],on;if(y=W,this.options.pedantic?(y=y.replace(this.rules.other.listReplaceNesting,"  "),on=y):on=y.replace(this.rules.other.tabCharGlobal,"    "),A.test(y)||I.test(y)||tn.test(y)||S.test(y)||j.test(y))break;if(on.search(this.rules.other.nonSpaceChar)>=g||!y.trim())c+=`
`+on.slice(g);else{if(w||p.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||A.test(p)||I.test(p)||j.test(p))break;c+=`
`+y}!w&&!y.trim()&&(w=!0),h+=W+`
...
`;return i?'<pre><code class="language-'+An(i)+'">'+(t?o:An(o,!0))+`</code></pre>
...
//# debugId=94D9C9AE572CF24264756E2164756E21
