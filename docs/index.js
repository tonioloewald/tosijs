...
`,n=n.substring(b.length+1),l=!0),!l){let O=this.rules.other.nextBulletRegex(w),x=this.rules.other.hrRegex(w),j=this.rules.other.fencesBeginRegex(w),S=this.rules.other.headingBeginRegex(w),N=this.rules.other.htmlBeginRegex(w);for(;n;){let bn=n.split(`
`,1)[0],an;if(b=bn,this.options.pedantic?(b=b.replace(this.rules.other.listReplaceNesting,"  "),an=b):an=b.replace(this.rules.other.tabCharGlobal,"    "),j.test(b)||S.test(b)||N.test(b)||O.test(b)||x.test(b))break;if(an.search(this.rules.other.nonSpaceChar)>=w||!b.trim())c+=`
