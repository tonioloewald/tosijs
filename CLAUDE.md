...
> **The debug/safe bundles are EXPERIMENTAL and currently inert.** `tjs convert` marks every converted-from-TS function `unsafe` (TS is presumed tsc-checked; safety is opt-in in native TJS), so **no runtime type checks fire on this line yet** — the bundles ship complete per-function `__tjs` metadata and wired config, but enforcement arrives only as modules go native-TJS in 2.0. The debug bundle announces this on load. **Gotcha:** the config *must* evaluate before any library module captures `globalThis.__tjs` — converted modules snapshot `__tjs?.createRuntime?.()` at eval time. That's why config lives in `configure-tjs-*` modules imported before `export * from './index'`; the old inline config landed *after* the library had already evaluated (H-4). State-update type checking (`settings.strictness`) is a *different* axis — 2.0-branch only, see TODO.
