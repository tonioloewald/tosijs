...
> **🛑 tjs-lang work is on HOLD until 0.13.0 stable** (maintainer, 2026-08-21).
> 0.13.0 is not an increment — the language was **reoriented to be TypeScript
> plus obvious improvements** rather than to fight TS idioms and muscle memory,
> so what `convert` emits, what a native-TJS module looks like, and how much of
> the 2.0 port's recorded friction survives are all open questions. We stay on
> `tjs-lang@0.10.1` and, consequently, on `tosijs-ui@1.9.4`.
>
> ⚠️ **The reason once given for the tosijs-ui pin was false.** It said 1.10.0
> was unadoptable because it peers `tjs-lang ^0.12.0`. Verified against the
> registry: **1.9.1, 1.9.4 and 1.10.0 all declare that peer**, including the
> 1.9.4 we install — so the bump was never blocked by it, and the `watchPaths`
> duplication (the tosijs-ui#49 workaround, #49 now closed upstream) was paid
> for nothing. It is deferred for one honest reason: it is a build-host change
> and 1.8.0 is mid-release. Details: `UPSTREAM.md` § tjs-lang, and `TODO.md` E1.
