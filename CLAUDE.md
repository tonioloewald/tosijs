...
> threw `Cannot call a class constructor without |new|` — at *import* time where
...
> 0.13.6** (tjs-lang#37, closed): the transform moved to the *graduation* step,
...
Deprecated APIs emit a single `console.warn` per feature (tracked in a `Set` in `metadata.ts`). Old names (`xinValue`, `xinPath`, `tosiValue`, `tosiPath`) still work but should not be used in new code. Prefer `.tosi.value` and `.tosi.path` (or `.value`/`.path` when no shadowing risk). The direct properties on boxed proxies (`.value`, `.path`, `.observe()`, etc.) delegate through the same accessor implementation as `.tosi` but are not deprecated — they're convenient when there's no collision risk. **A `bind*` element-prop shortcut is deprecated IFF a plain prop expresses it exactly — which depends on the VALUE, not just the key.** With a **proxy**, `bindText` → `textContent` and `bindDisabled`/`bindEnabled` → `disabled` (inverted) are exact, so those warn. With a **path string** none of them is: `textContent: 'path'` sets literal text and `disabled: 'path'` assigns an always-truthy string that permanently disables the control — so the string form does **not** warn (1.9.0; tosijs-ui reached the same conclusion for `bindText` independently). **`bindValue` and `bindList` never warn** — one is two-way (`value: proxy` is one-way only), the other needs a `<template>` sibling and options. `.tosi.listBinding()` is *sugar over* `bindList`, so deprecating it made the recommended API warn its own callers from inside itself, and its message could not even be written as a props key. Routing around that instead of questioning it produced a silent list-destroying `bind` collision and a breaking change to a public return shape; see `reviews/`.
