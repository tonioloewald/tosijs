...
> **🛑 `tjs-lang` is pinned at `0.12.0`, and NOT because of a peer range**
> (2026-08-26). **0.13.x's `convert` is broken for us**: it strips `new` from
> every class declared in the module being converted, so the output throws
> `Cannot call a class constructor without |new|` — at *import* time when the
> call site is a static field initialiser. 15 sites across 4 modules,
> `UnsafePathError` (the prototype-pollution guard) among them. Bisected to
> **0.13.0**; **0.12.0 is the last good version**. Filed as **tjs-lang#37**.
> 0.13.4 also emits a syntax error for `component.ts`. Do not bump until #37
> closes. `tosijs-ui` is on **1.12.0** and declares `tjs-lang` an *optional*
> peer, so `^0.13.1` vs our 0.12.0 is a warning, not a break.
>
> **`bun install` will report `tjs-lang@0.12.0` as deprecated. That is expected
> and deliberate** — every non-deprecated version carries the #37 bug, and the
> deprecation reason (a tosijs-schema/battery-atoms interaction) does not reach
> `convert`, which is all we use. Do not "fix" it by bumping.
>
