...
The transport slot is being standardized _right now_ — and the core idea remains
unclaimed:
...
- **WebMCP** (`navigator.modelContext`, migrating to `document`): a W3C WebML CG
  ...
  possible") — but it is a _manual registration API_: every tool is hand-written.
...
- **Playwright MCP / Operator / Computer Use / Mariner:** a11y-tree + vision +
  ...
  **Scope honesty:** their actual value proposition is working on _arbitrary_
  ...
  isn't tosijs.
- **Phoenix LiveView / Hotwire:** real prior art for the _inversion_ (app lives
...
1. **State, not just tools.** WebMCP exposes callable functions; nobody exposes a
   path-addressable, _writable, observable_ model.
2. **Derived, not declared.** Every integration above hand-registers tools.
   tosijs can _generate_ the surface from wiring it already records — because no
...
shipping in Chrome, and tosijs can be _the first framework where the WebMCP
tools write themselves_. haltija/MCP remains the second transport (and covers
