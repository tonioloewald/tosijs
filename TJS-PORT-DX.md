...
## Restart on 1.8.0 (2026-08-26) — read this first

The branch was **reset from `main` at 1.8.0** and the old history archived at tag
`tosijs-2.0-archive-2026-08-26`. It was 198 commits behind with a merge base at
v1.7.0, and a rebase was not merely painful but *unsafe*: `by-path.ts` is deleted
on the old branch and was modified by four commits on main, three of them the
1.8.0 path-security work. The archived `by-path.tjs` contains **zero** of
`assertSafeKey` / `assertSafeLeafKey` / `UnsafePathError`, so resolving that
delete/modify the obvious way silently reverts the prototype-pollution guard.
Anything wanted from the old branch should be **re-derived from current main**,
never replayed. (`path-check.ts`, the strictness work and the boxed-scalar
equality tests are still in the archive tag and were not brought forward.)

**What changed underneath the whole premise.** This log's standing verdict was
that `by-path.tjs` "costs +1.2% gzip and delivers *zero* value." That was true
when written on 2026-07-13 — `__tjs` metadata was emitted and **nothing consumed
it**. 1.8.0 shipped the agent surface, whose entire thesis is that an app already
records its affordances and should let you ask for them. `__tjs` is that same
idea for *functions*, which is the half tosijs records worst: `describe()`
returns `actions: string[]` — bare path strings — so what an action *takes* has
to be a hand-authored contract that is free to drift from the signature it
claims to describe. **The port's headline is that contracts become derived
rather than authored**, and that claim did not exist when the port was assessed.
The agent surface does not read `__tjs` today (grep across `agent.ts`,
`contract*.ts`, `webmcp.ts` is empty) — that connection is unbuilt, and it is
the highest-value thing on this branch.

Also settled: **the unit of adoption is a FILE, not the library.** Convert one
module, take whichever of introspection / inline tests / safety guards you want,
and nothing else in the graph knows. That dissolves "is the port worth it" as a
question — there is no port, only files.

### First native module on 1.8.0: `more-math.tjs` — what it actually cost

Deliberately the smallest real module (`clamp`, `lerp`, two constants), converted
end to end: 898 unit tests pass, full build exits 0 with every gate green, and
the Playwright lane is 4/4.

**1. Inline tests work, and they turned decorative docs into load-bearing ones.**
The doc block had carried `clamp(0, 0.5, 1) // produces 0.5` since forever, in a
bare fence nothing runs. Those examples are now the signature (`clamp(min: 0.0,
v: 0.5, max: 1.0): 0.5`) plus `test { }` blocks, all executed by `tjs test`.
Verified by breaking it: claiming `: 0.9` fails with `Expected 0.9 at 'clamp',
got 0.5`. The `clamp` NaN-on-inverted-bounds behaviour had been *documented and
unasserted* for the life of the library; it is asserted now.

**2. CONVERT FIRST, THEN IMPROVE — do not hand-author the `.tjs`.** This module
was hand-written first, and that was the mistake. In TJS **the example IS the
type**: `0` infers `kind: "integer"` and installs a `Number.isInteger` guard,
`0.0` infers `number`. Hand-written as `lerp(a: 0, b: 10, …)` it compiled happily
and then **refused `lerp(0.5, -0.5, 3, false)`** — a real call from the existing
suite — because a placeholder had been picked for looking tidy.

**`tjs convert --emit-tjs` never makes that mistake**, because TypeScript already
said `number`, so it emits `0.0`. The types come across free and correct; only a
human writing examples by eye gets them wrong. Recorded here as a hazard of the
wrong workflow, NOT as a language footgun — the earlier version of this entry had
it the other way round and was wrong.

What convert *cannot* do is know an answer, so it marks every return `:!` — "this
example describes the type, not the result." Nothing in a TS signature says what
`clamp(0.0, 0.5, 1.0)` produces. **So the port workflow is exactly two steps:**

1. `tjs convert --emit-tjs` — types, mechanically, correct.
2. Upgrade `:!` → `:` wherever a real worked example exists (turning the return
   into an executed assertion), and add `test { }` blocks.

Step 2 is the entire value-add, and it is precisely the part TypeScript could not
express — which is the sharp form of this log's standing rule that a mechanical
transpile delivers nothing. The transpile is not the deliverable; it is the
*cheap correct starting point* that leaves you free to spend your judgement on
the assertions. `more-math.tjs` on this branch is built that way and says so in
a comment, so the next module does not relearn it.

Note this is also the first time runtime type checks have fired in this project
at all: converted-from-TS functions are all marked `unsafe`, native ones are not.

**3. The size cost, measured — and it inverts the obvious advice.**
...
| core.js | +976 |
| state.js | +1064 |
| module.debug.js | +1033 |
| module.safe.js | +1043 |

Uniform, because it is an **entry fee**, not per-module weight: every bundle
carries its own copy of the self-contained `__tjs` runtime. Measured directly —
one `.tjs` module in a bundle is 1_364 gz, two is 1_718, so an equivalent second
module adds **~354**. And `tjs emit --unsafe` (which strips `__tjs` metadata)
saves only **181 raw bytes**, so the weight is the runtime and the guards, *not*
the introspection data.

**~1 kB to open the door, ~350 per room.** So converting ONE file is the worst
ratio you will ever measure, and "try one and see" produces a number that argues
against the thing it was meant to evaluate. Do not quote a single-module delta as
the cost of the port.

`index.js` is the artifact to watch: the CDN IIFE cannot tree-shake — which is
why it already omits the agent surface — and it had ~200 bytes of headroom. If
the port lands, that entry point may have to opt out. Branch budgets are raised
~1.5 kB across the board; **do not port the numbers to main, port the
measurement.**

**4. Converting a module SILENTLY DELETES ITS DOCS PAGE.** `docs/more-math`
vanished; the build exited 0 and the internal-link check *passed* at 41 slugs
(down from 42) because it verifies that links resolve, and a page that was never
generated is linked from nowhere. Cause: `site/docs.js:215` scrapes `/*# … */`
from `['.ts', '.js', '.css']` only, and `make-llms-txt.js:63` filters
`.endsWith('.ts')`. Filed as **tosijs-ui#108**, including a suggestion to report
the slug count as a delta so any silent page loss is visible.

**5. Small gaps worth knowing.** tjs's inline `expect` has no `toBeCloseTo`, so
float comparison is written out by hand — worth knowing before porting anything
numerics-heavy, which is most of why you would reach for this. And every `.tjs`
import must write the extension (Bun's *runtime* loader ignores plugin
`onResolve` for relative specifiers — a Bun limitation, unchanged).

### Not attempted: inline wasm

No honest home for it in tosijs. The only arithmetic here is colour-space
conversion and virtual-list geometry, neither hot enough to beat the call
overhead — `lerp` in wasm would be *slower* than `lerp` in JS. tjs-lang's own
wasm example is literally `lerp`, which makes it a fine demo and a bad argument.
The real consumers are tosijs-3d and manta-recon, where the data is numerical and
